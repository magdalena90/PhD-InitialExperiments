---
title: 'WGCNA tree cutting methods comparison'
output:
  html_document:
    code_folding: 'hide'
---
...
- Extracting clusters using a dynamic brach cutting approach from [Dynamic Tree Cut: in-depth description, tests and applications](https://horvath.genetics.ucla.edu/html/CoexpressionNetwork/BranchCutting/Supplement.pdf) **comparing fixed height cut vs Dynamic Tree and Dynamic Hybrid**
...
### Fixed height cut (for comparison)

The root of the dendrogram seems to separate genes almost one by one until in separates them into two big groups at around 0.9915, after which it continues to separate genes almost one by one. Using a height of 0.9913 and removing all the modules with less than 20 clusters (as the dynamic tree cut suggests) we are left with only 4 of the 157 modules found

```{r, fig.width=10, warning=FALSE, message=FALSE}
dend %>% as.dendrogram %>% plot(ylim=c(0.99,1))
abline(h=0.9913, col='blue')

modules = cutree(dend, h=0.9913)
modules[!modules %in% names(table(modules)[table(modules)>20])] = 0

names(modules) = labels(dend)

clusterings[['fixed_height_tree']] = modules
```
...
top_modules = modules %>% replace(modules %in% (gsub('ME', '', names(tree_cut[tree_cut==1])) %>% as.numeric), 1) %>%
                          ...
                          replace(modules == 0, 0)
clusterings[['fixed_height_tree_top_clusters']] = top_modules
```
...
rm(MEs, modules, module_colors, MEs_output, top_modules, tree_cut, top_module_colors, cor_dist, dend_colors, dend_MEs)
```
<br><br>
...
rm(MEs, modules, module_colors, MEs_output, top_modules, merged_modules, tree_cut, top_module_colors, 
   merged_module_colors, cor_dist, dend_colors, i, dend_MEs, best_power)
```
<br><br>
...
heatmap.2(x = cluster_sim, Rowv = F, Colv = F, dendrogram = 'none', col=rev(brewer.pal(9,'YlOrRd'))[4:9],
          ...
          cexRow = 1, cexCol = 1, margins = c(14,14))
 

rm(i, j, cluster1, cluster2, cluster_sim, cor_mat, S, S_sft, TOM, dissTOM)
```
...
plot_data = data.frame('ID' = rownames(datExpr), 'PC1' = pca$x[,1], 'PC2' = pca$x[,2], 
                       ...
                       'fixed_height_tree' = sub(0,NA,clusterings[['fixed_height_tree']]) %>% as.factor,
                       'fixed_height_tree_top_clusters' = sub(0,NA,clusterings[['fixed_height_tree_top_clusters']]) %>% as.factor,
...
- The fixed height cut tree gives moules more similar to the dynamic tree algorithm than to the dynamic hybrid, but finds very few modules and leaves some genes whitout classification
...
- **I'm going to use the Dynamic tree approach because it has less parameters to tune, it's supposed to be more robust and although it leaves more genes without classification, the modules seem to be better defined in the dendrogram**
