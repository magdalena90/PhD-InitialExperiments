...
- Chose this threshold based on the valleys formed in the density plot. The next big valley is formed at 48.5, but that would leave only 14576 genes
...
Found 14 surrogate variables, since there is no direct way to select which ones to pick [Bioconductor answer](https://support.bioconductor.org/p/71447/), kept all of them.
...
# counts and rowRanges already defined in the SVA analysis
se = SummarizedExperiment(assays=SimpleList(counts=counts), rowRanges=rowRanges, colData=datMeta_sva)
dds = DESeqDataSet(se, design = ~ Batch + SV1 + SV2 + SV3 + SV4 + SV5 + SV6 + SV7 + SV8 + SV9 + 
...
There seems to still be a difference in the behaviour in the first PC to the processing date, although not as big as before
...
No recognisable difference in this plots
```{r, warning=FALSE, message=FALSE}
pca = datExpr %>% t %>% prcomp
summary(pca)$importance[,1:3]
