...
print(paste0('Removed ', sum(!keep), ' samples, ', sum(keep), ' remaining'))
...
$\qquad$ 2.2 Removing genes with a mean expression lower than 1.5
...
- Outliers come from the Harvard site, are all ASD samples and belong to relativelty young subjects, but it could be just a coincidence
...
Found 24 surrogate variables, since there is no direct way to select which ones to pick [Bioconductor answer](https://support.bioconductor.org/p/71447/), kept all of them.
...
For now, I won't do this because of the correlation between processing site and diagnosis
