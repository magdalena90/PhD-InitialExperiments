...
cat(paste0('Best power for scale free topology: ', best_power$powerEstimate))
```

### Create Adjacency Matrix

I was originally using an unsigned (|cor|) adjacency function (because originally there were only signed and unsigned options), but there is now a third option, called signed hybrid (max(0,cor)), which Peter Langfelder recommends in his blog entry [Signed or Unsigned: which network type is preferable? ](https://peterlangfelder.com/2018/11/25/signed-or-unsigned-which-network-type-is-preferable/) where he says that:
...
- Negatively correlated genes tend to come from biologically very different categories (and by using and unsigned adjacency function you are grouping themm all together)

So I switched to this one.

<br>

Using biweight midcorrelation instead of regular correlation because it's more robust to outliers
...
**MOST** of genes (83%) are left without a cluster! (With Gandal's dataset it was less than half and it was still bad)
...
abline(h=0.6, col='#009999')
abline(h=0.3, col='#99cc00')

merge_top = datExpr %>% t %>% mergeCloseModules(modules, cutHeight = 1)

merge_similar = datExpr %>% t %>% mergeCloseModules(modules, cutHeight = 0.6)
...
Only the genes farthest away from the root of the tree are assigned a module. This is not very useful
