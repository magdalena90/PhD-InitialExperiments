---
title: 'WGCNA manually condensed'
output:
  html_document:
    code_folding: 'hide'
---
...
for(i in unique(tree_cut)){
  merged_modules = modules %>% replace(modules %in% (gsub('Mod','', names(tree_cut[tree_cut==i])) %>% as.numeric), i)
}
...
rm(MEs, dend, modules, module_colors, MEs_output, top_modules, merged_modules, tree_cut, top_module_colors, dend_colors)
```
<br><br>
...
bicor_dist = 1-bicor(MEs)
dend_MEs = bicor_dist %>% as.dist %>% hclust(method='average')
dend_MEs %>% as.dendrogram %>% set('labels', rep('', nrow(bicor_dist))) %>% plot(ylim=c(0, 0.7))
