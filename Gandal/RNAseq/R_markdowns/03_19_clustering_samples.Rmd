...
Younger ASD samples seem to be more similar to control samples than older ASD samples (pink cluster has most of the youngest samples). The yellow cluster is made of young ASD samples. Most old ASD samples are also close together.
...
Samples are grouped into two big clusters and a small third cluster with 5 samples, and then the two big clusters have 5 and 8, respectively.

*Output plots in clustering_samples_03_15 folder
```{r echo=FALSE, message=FALSE}
cc_output = datExpr_redDim %>% as.matrix %>% t %>% 
  ConsensusClusterPlus(maxK=5, reps=50, seed=123, title='clustering_samples_03_19/cc_l1/', plot='png')
best_k = 3
...
cc_output_c1 = datExpr_redDim %>% filter(clusterings[['cc_l1']]==1) %>% as.matrix %>% t %>% 
  ConsensusClusterPlus(maxK=8, reps=50, seed=123, title='clustering_samples_03_19/cc_l2_1/', plot='png')
best_k = 5
...
cc_output_c2 = datExpr_redDim %>% filter(clusterings[['cc_l1']]==2) %>% as.matrix %>% t %>% 
  ConsensusClusterPlus(maxK=10, reps=50, seed=123, title='clustering_samples_03_19/cc_l2_2/', plot='png')
best_k = 8
...
3. Leaves most of the observations (55) without a cluster:
```{r}
...
It finds a single cluster grouping 60 observations and leaves the rest without cluster:
```{r}
...
* GMM CC_l1 are the most similar

* ICA seems to be clustering individuals together, but not as much as last time

* CC_l1 is the one with the strongest relation to ASD
```{r}
